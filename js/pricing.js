const PRICES = [
  ...
  //   price: "$49",
  //   title: "Basic",
  //   description: "Low cost way for hobbyist to try things out.",
  //   token: "Comes with 150 tokens*",
  //   popular: false,
  //   basic: true
  // },
  {
  ...
  },
  {
    price: "$299",
    title: "Pro",
    description: "For high-volume or complex workflows",
    token: "Comes with 8,000 tokens*",
    popular: false,
    basic: false
  },
];
...
function build(entry){
  const {price, title, description, token, popular, basic} = entry;
  ...
  a.innerHTML = "Request early access";
  a.href = "#early-access";

  const bgColor = popular ? "bg-blue-600" : basic ? "bg-slate-800" : null;
  ...
  section.classList.add("flex", "flex-col", "sm:rounded-3xl", "px-6", "sm:px-8", "py-8", bgColor);
